...
describe('Firestore collection', () => {
	...
	it('Should create document with specific ID', (done) => {
		const refId = '0xxx0';
		Supplier.create(
			{
				name: 'ACME inc',
			...
			(err, supplier) => {
				// expect(supplier.id).to.equal(refId);
				supplier.should.have.property('id', refId);
				done(err);
			}
		);
	});
	...
	it.skip('Should get all documents', (done) => {
		Customer.all((err, customers) => {
	...
	it.skip('Should find a documents by age less than 28', (done) => {
	...
	it.skip('Should get Cristian as first Document in the array', (done) => {
	...
	it.skip('Should Replace attributes for a model instance', (done) => {
	...
	it.skip('Should delete a document', (done) => {
		Customer.destroyAll({ id: customer1.id }, (err) => {
			done(err);
		});
	});

	it.skip('Should delete all document', (done) => {
